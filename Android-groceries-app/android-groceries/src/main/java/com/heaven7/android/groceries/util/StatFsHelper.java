...
public class StatFsHelper {
    ...
    public static final byte STORAGE_TYPE_INTERNAL = 1;
    public static final byte STORAGE_TYPE_EXTERNAL = 2;
 ...
 // @GuardedBy("lock")
   ...
   */
  public boolean testLowDiskSpace(@StorageType byte storageType, long freeSpaceThreshold) {
    ensureInitialized();
  ...
  public long getAvailableStorageSpace(@StorageType byte storageType) {
    ensureInitialized();

    maybeUpdateStats();

    StatFs statFS = storageType == STORAGE_TYPE_INTERNAL ? mInternalStatFs : mExternalStatFs;
    if (statFS != null) {
